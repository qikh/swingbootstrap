...
public class ButtonGroup extends JPanel {
	...
	public int setButtons(List<Button> navLinkList) {
		for (Button btn : navLinkList) {
	...
	private void setLeftNode() {
		if (leftLinkList.size() > 0) {
			...
			if (comp instanceof Button) {
				((Button) comp).setLeftNode(true);
			}

			JComponent lastComp = leftLinkList.get(leftLinkList.size() - 1);
			if (lastComp instanceof Button) {
				((Button) lastComp).setRightNode(true);
			}
		}
	}
