...
public class NavlinkUI extends BasicButtonUI {
	...
	protected void paintButtonPressed(Graphics g, AbstractButton b) {
		if (b.isContentAreaFilled()) {
			if (b instanceof Navlink) {
				Navlink link = (Navlink) b;
				if (link.isLeftNode()) {
	...
	protected void paintFocus(Graphics g, AbstractButton b, Rectangle viewRect, Rectangle textRect, Rectangle iconRect) {
		if (b instanceof Navlink) {
			...
			if (link.isLeftNode()) {
				...
				// create round rectangle area and fill it with color
				RoundRectangle2D rect = new RoundRectangle2D.Float(0, 0, width, height, link.getNavbar().getArc(), arc);
